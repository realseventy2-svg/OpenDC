...
# Add addon directory to sys.path and register
addon_dir = os.path.abspath("tools/blender_addon")
...
# 1. Clear existing objects in the default scene (without destroying workspace layout)
bpy.ops.object.select_all(action='SELECT')
bpy.ops.object.delete(use_global=False)

# Clean up existing materials and node groups
...
cam_obj.location = (0.0, -4.8, 2.8)
cam_obj.rotation_euler = (1.05, 0.0, 0.0) # Look down at console
scene.camera = cam_obj

# 3. Add Key & Fill Lights
...
light_obj.location = (2.5, -3.0, 4.0)
...
# Helper function to create Principled BSDF materials with vibrant viewport colors
...
mat_dc_dark   = create_colored_mat("Mat_DC_Dark",    (0.15, 0.15, 0.18, 1.0), 0.5)
mat_dc_white  = create_colored_mat("Mat_DC_White",   (0.95, 0.95, 0.95, 1.0), 0.2)
mat_text_dark = create_colored_mat("Mat_Text_Dark",  (0.08, 0.08, 0.10, 1.0), 0.1)
mat_text_light= create_colored_mat("Mat_Text_Light", (1.0, 1.0, 1.0, 1.0), 0.1)

# 4. Create 3D Dreamcast Console Base
...
# Disc Lid Circle
bpy.ops.mesh.primitive_cylinder_add(radius=0.75, depth=0.1, location=(0.0, 0.1, 0.12))
...
# Dreamcast Swirl Accent / Power Light
...
# 5. Create 3D Interactive Buttons
buttons_info = [
    ("Btn_Boot_Disc",   (-2.0,  0.8, 0.45), "Boot GD-ROM",   mat_dc_orange),
...
# 6. Create OpenDC Visual Logic Tree ("Blueprint")
...
# 7. Configure Workspace UI Layout (Split Screen: 3D Viewport + OpenDC Logic Nodes)
...
# 8. Save .blend file
blend_out = "tools/demo_bios_menu.blend"
bpy.ops.wm.save_as_mainfile(filepath=blend_out)
print(f"Successfully saved demo .blend file: {blend_out}")

# 9. Export to .dcui binary containers
from io_scene_dcui import exporter
dcui_out = "bios/res/ui/default_menu.dcui"
exporter.export_scene_to_dcui(dcui_out, scene)
exporter.export_scene_to_dcui("bios/romdisk/menu.dcui", scene)
print(f"Exported .dcui from Blender scene directly to {dcui_out}")
