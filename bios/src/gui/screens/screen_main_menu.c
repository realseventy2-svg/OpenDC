...
void screen_main_menu_render(void) {
    ...
    /* Title using Authentic Sega BFont */
    draw_bfont(start_x, y, COLOR_WHITE, "DREAMCAST BOOTMENU");
    y += 28;
    ...
    for(int i = 0; i < MENU_COUNT; i++) {
        if(i == s_menu_sel) {
            draw_bfont_fmt(start_x, y, COLOR_WHITE, "> %s", MENU_ITEMS[i]);
        } else {
            draw_bfont_fmt(start_x, y, COLOR_LIGHT_GRAY, "  %s", MENU_ITEMS[i]);
        }
        y += 28;
    }
    ...
    /* Developer status comments using monospace SysFont */
    if(disc->disc_present) {
        draw_sysfont_fmt(start_x, y, COLOR_GRAY,
            ".// Disc: [%s] %s (%s)",
            disc->is_gdrom ? "GD-ROM" : "CD-ROM",
            disc->title,
            disc->product_id[0] ? disc->product_id : "HDR-XXXX");
    } else {
        draw_sysfont(start_x, y, COLOR_GRAY, ".// Disc: No Disc Inserted in Drive [STANDBY]");
    }
    y += 16;

    draw_sysfont(start_x, y, COLOR_GRAY,
    ...
    /* Controls footer (centered, fits cleanly within 640px) */
