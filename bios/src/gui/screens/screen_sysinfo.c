...
void screen_sysinfo_render(void) {
    ...
    draw_text_fmt(start_x + 16, y, COLOR_LIGHT_GRAY, "- %s @ %s", specs->cpu_model, specs->cpu_speed); y += 14;
    draw_text_fmt(start_x + 16, y, COLOR_LIGHT_GRAY, "- %s", specs->cpu_cache); y += 14;
    draw_text(start_x + 16, y, COLOR_GREEN, "- Status: [ PASS / OPERATIONAL ]"); y += 20;

    draw_text(start_x, y, COLOR_WHITE, "MEMORY ARCHITECTURE:"); y += 15;
    draw_text_fmt(start_x + 16, y, COLOR_LIGHT_GRAY, "- %s (%s)", specs->ram_size, specs->ram_bus); y += 14;
    ...
    draw_text_fmt(start_x + 16, y, COLOR_LIGHT_GRAY, "- %s @ %s", specs->gpu_model, specs->gpu_speed); y += 14;
    draw_text_fmt(start_x + 16, y, COLOR_LIGHT_GRAY, "- %s", specs->vram_size); y += 14;
    draw_text_fmt(start_x + 16, y, COLOR_LIGHT_GRAY, "- Active Output: %s", sysinfo_get_cable_name()); y += 20;

    draw_text(start_x, y, COLOR_WHITE, "AUDIO & BUS SUBSYSTEM:"); y += 15;
    draw_text_fmt(start_x + 16, y, COLOR_LIGHT_GRAY, "- %s", specs->audio_chip); y += 14;
    draw_text_fmt(start_x + 16, y, COLOR_LIGHT_GRAY, "- %s", specs->audio_cpu); y += 24;
